...
pub fn handle_args() -> Result<(Gadget, Scheme, Curve, String), String> {
...
macro_rules! handle_curve {
    ($func_name:ident, $rng_name:ident, $c:expr, $rng:expr) => {
        match $c {
            Curve::Bls12_381 => $func_name::<curve::Bls12_381, $rng_name>($rng),
            Curve::Bn_256 => $func_name::<curve::Bn_256, $rng_name>($rng),
        }
    };
}

macro_rules! handle_gadget {
    ($gadget:ident, $rng_name:ident, $s:expr, $c:expr, $rng:expr) => {
        match $s {
            Scheme::Groth16 => {
                use $gadget::groth16_setup;
                handle_curve!(groth16_setup, $rng_name, $c, $rng)
            }
...
fn main() -> Result<(), String> {
    let (g, s, c, filename) = handle_args()?;
    ...
    let (pk, vk) = match g {
        Gadget::MiMC => handle_gadget!(mimc, ThreadRng, s, c, rng)?,
    };

...
use rand;
use rand::Rng;
use math::{PairingEngine, PrimeField,  UniformRand, Zero, One};
use math::fft::{DensePolynomial as Polynomial, DenseOrSparsePolynomial};
use math::fft::EvaluationDomain;
use math::msm::{FixedBaseMSM,VariableBaseMSM};
use curve::{AffineCurve, ProjectiveCurve};
use core::ops::{Deref, Div, Sub, Mul, Add, Neg, SubAssign,MulAssign, AddAssign};
...
R: Rng,
{
    ...
    let g1_table =
        FixedBaseMSM::get_window_table::<E::G1Projective>(scalar_bits, g1_window, g1);

    let g2_window =
        FixedBaseMSM::get_mul_window_size(max_degree + 1);
    let g2_table =
            FixedBaseMSM::get_window_table::<E::G2Projective>(scalar_bits, g2_window, g2);
    ...
    for i in 0..max_degree{
        ...
        // 1/nω^(n-i) = ω^i/n
        let a_aside_omega_i_divisor = domain.group_gen.pow(&[i as u64]).div(&E::Fr::from_repr((max_degree as u64).into()));
...
E: PairingEngine,
{
    ...
    let commit = VariableBaseMSM::multi_scalar_mul(&prk_params.l_of_g1.clone(), values.into_iter().map(|e| e.into_repr()).collect::<Vec<_>>().as_slice());
...
E: PairingEngine,
{
    ...
    let domain = EvaluationDomain::<E::Fr>::new(prk_params.powers_of_g1.len() - 1).ok_or(SynthesisError::PolynomialDegreeTooLarge)?;
    ...
    for point in points.iter() { 
        let tpoly = Polynomial::from_coefficients_vec(vec![domain.group_gen.pow(&[*point as u64]).neg(),E::Fr::one()]);
    ...
    let (witness_polynomial, _) = dense_or_sparse_poly.divide_with_q_and_r(&dense_or_sparse_divisor).unwrap();

    // π = g_1^q(τ)
    let witness = VariableBaseMSM::multi_scalar_mul(&prk_params.powers_of_g1.clone(), &witness_polynomial.deref().to_vec().into_iter().map(|e| e.into_repr()).collect::<Vec<_>>());
...
E: PairingEngine,
{
    ...
    for point in points.iter(){
        let tpoly =  Polynomial::from_coefficients_vec(vec![omega.pow(&[*point as u64]).neg() , E::Fr::one()]);
    ...
    let r_value = VariableBaseMSM::multi_scalar_mul(&vrk_params.powers_of_g1.clone(), &r_polynomial.deref().into_iter().map(|e| e.into_repr()).collect::<Vec<_>>());
    ...
    // A_I(τ) = ∏(τ - ω^i)
    let a_value = VariableBaseMSM::multi_scalar_mul(&vrk_params.powers_of_g2.clone(), &a_polynomial.deref().to_vec().into_iter().map(|e| e.into_repr()).collect::<Vec<_>>());
...
E: PairingEngine,
{
    ...
    // g^τ / g^(ω^i)
    let inner = vrk_params.powers_of_g2[1].into_projective().sub(&vrk_params.powers_of_g2[0].into_projective().mul(omega_i));
    ...
    //a_i^(1/A'(ω^i))
    let a_aside_omega_i_divisor = omega.pow(&[point as u64]).div(&E::Fr::from_repr((n as u64).into()));
...
E: PairingEngine,
{
    ...
    if point_i == point_j {
        ...
        // u_ij = w_ij ^ (1/A'(w^j))
        let a_aside_omega_i_divisor = omega.pow(&[point_j as u64]).div(&E::Fr::from_repr((n as u64).into()));
...
E: PairingEngine,
{
    ...
    for (point, proof) in points.iter().zip(proofs.iter()){
        let divisor_polynomial = Polynomial::from_coefficients_vec(vec![omega.pow(&[*point as u64]).neg(),E::Fr::one()]);

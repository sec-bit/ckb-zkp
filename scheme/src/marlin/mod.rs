...
use crate::r1cs::ConstraintSynthesizer;
...
pub struct Marlin<E: PairingEngine, D: Digest> {
    _engine: PhantomData<E>,
    _digest: PhantomData<D>,
}

impl<E: PairingEngine, D: Digest> Marlin<E, D> {
    pub const PROTOCOL_NAME: &'static [u8] = b"MARLIN";
    ...
    pub fn universal_setup<R: RngCore>(
        ...
        let max_degree = AHP::<E::Fr>::max_degree(num_constraints, num_variables, num_non_zeros)?;
    ...
    pub fn prove<R: RngCore, C: ConstraintSynthesizer<E::Fr>>(
        ...
        let mut fs_rng = FiatShamirRng::<D>::from_seed(
            &to_bytes![&Self::PROTOCOL_NAME, &ipk.index_verifier_key, &public_input].unwrap(),
        );
        // first round
        ...
        // second_round
        let (pstate, second_oracles) =
            AHP::<E::Fr>::prover_second_round(pstate, &verifier_first_msg)?;
        ...
        let (vstate, verifier_second_msg) =
            AHP::<E::Fr>::verifier_second_round(vstate, &mut fs_rng)?;
        // third_round
    ...
    pub fn verify(
        ...
        let mut fs_rng = FiatShamirRng::<D>::from_seed(
            &to_bytes![&Self::PROTOCOL_NAME, &ivk, &public_input].unwrap(),
        );

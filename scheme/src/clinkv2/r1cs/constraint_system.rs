...
pub trait ConstraintSystem<F: Field>: Sized {
    ...
    fn alloc<FN, A, AR>(&mut self, annotation: A, f: FN, i: usize) -> Result<Variable, SynthesisError>
    where
    ...
    fn alloc_input<FN, A, AR>(&mut self, annotation: A, f: FN, i: usize) -> Result<Variable, SynthesisError>
    where
        FN: FnOnce() -> Result<F, SynthesisError>,
        A: FnOnce() -> AR,
        AR: Into<String>;
...
impl<F: Field, CS: ConstraintSystem<F>> ConstraintSystem<F> for Namespace<'_, F, CS> {
    ...
    where
        ...
        AR: Into<String>,
    {
        self.0.alloc_input(annotation, f, i)
    }
...
impl<F: Field, CS: ConstraintSystem<F>> ConstraintSystem<F> for &mut CS {
    ...
    where
        ...
        AR: Into<String>,
    {
        (**self).alloc_input(annotation, f, i)
    }

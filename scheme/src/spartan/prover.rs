...
> {
    ...
    for i in 0..num_rounds {
        ...
        let poly_a_tmp = combine_with_n::<E>(&poly_a.to_vec(), E::Fr::from(2u32));
        let poly_b_tmp = combine_with_n::<E>(&poly_b.to_vec(), E::Fr::from(2u32));
        let poly_c_tmp = combine_with_n::<E>(&poly_c.to_vec(), E::Fr::from(2u32));
        let poly_eq_tmp = combine_with_n::<E>(&poly_eq.to_vec(), E::Fr::from(2u32));
        let eval_2: E::Fr = (0..size)
            .map(|j| poly_eq_tmp[j] * &(poly_a_tmp[j] * &poly_b_tmp[j] - &poly_c_tmp[j]))
            .sum();
        // g_i(3) = eval_3 = 3eval_1 + (1-3)eval_0;
        let poly_a_tmp = combine_with_n::<E>(&poly_a.to_vec(), E::Fr::from(3u32));
        let poly_b_tmp = combine_with_n::<E>(&poly_b.to_vec(), E::Fr::from(3u32));
        let poly_c_tmp = combine_with_n::<E>(&poly_c.to_vec(), E::Fr::from(3u32));
        let poly_eq_tmp = combine_with_n::<E>(&poly_eq.to_vec(), E::Fr::from(3u32));
...
fn sum_check_proof_phase_two<E: PairingEngine, R: Rng>(
    ...
    for i in 0..num_rounds {
        ...
        // g_i(2) = eval_2 = 2eval_1 + (1-2)eval_0;
        let poly_abc_tmp = combine_with_n::<E>(&poly_abc, E::Fr::from(2u32));
        let poly_z_tmp = combine_with_n::<E>(&poly_z, E::Fr::from(2u32));
        let eval_2 = (0..size).map(|j| poly_abc_tmp[j] * &poly_z_tmp[j]).sum();

        // degree = 2
        // f(x) = ax^2 + bx + c
        // a = (eval_0 - 2eval_1 + eval_2)/2
        let a_coeff = (eval_0 - &eval_1.double() + &eval_2) * &E::Fr::from(2u32).inverse().unwrap();
        // c = eval_0
        let c_coeff = eval_0;
        // b = eval_1 - a - c

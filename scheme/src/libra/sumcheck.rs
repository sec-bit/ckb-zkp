...
pub fn sumcheck_phase_one<G: Curve>(
    f_vec: &Vec<G::Fr>,
    ...
    for _ in 0..bit_size {
        ...
        let a_coeff = (eval_0 - &eval_1.double() + &eval_2) * &G::Fr::from(2).inverse().unwrap();
        // c = eval_0
        let c_coeff = eval_0;
        // b = eval_1 - a - c
...
pub fn sumcheck_phase_two<G: Curve>(
    f_vec: &Vec<G::Fr>,
...
pub fn zk_sumcheck_phase_one<G: Curve, R: Rng>(
...
pub fn zk_sumcheck_phase_two<G: Curve, R: Rng>(
    params: &SumCheckCommitmentSetupParameters<G>,
    f_vec: &Vec<G::Fr>,
    g_vec: &(Vec<G::Fr>, Vec<G::Fr>, G::Fr),
    bit_size: usize,
...
pub fn sum_check_eval_prover<G: Curve, R: Rng>(
    params: &SumCheckCommitmentSetupParameters<G>,
    poly_size: usize,
    poly: &Vec<G::Fr>,
    comm_poly: G::Affine,
    ...
    let d_commit = poly_commit_vec::<G>(&params.gen_3.generators, &d_vec, &params.gen_3.h, r_delta);
...
pub fn sum_check_eval_verify<G: Curve>(
    params: &SumCheckCommitmentSetupParameters<G>,
    comm_poly: G::Affine,
    comm_eval: G::Affine,
    comm_claim: G::Affine,
    ...
    let sum: G::Fr = (0..bit_size).map(|i| proof.z[i] * &coeffs[i]).sum();
    let rhs = poly_commit_vec::<G>(
        &params.gen_1.generators,
        &vec![sum],
        &params.gen_1.h,

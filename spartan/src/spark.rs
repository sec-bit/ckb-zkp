...
use crate::{
    commitments::packing_poly_commit,
    data_structure::{
    ...
    r1cs::{switch_matrix_to_list, R1CSInstance},
    Vec,
};
...
pub fn memory_in_the_head<G: Curve>(
    ...
    for addrs in addrs_list.iter() {
        ...
        let read_ts = (0..read_ts.len())
            .map(|i| {
                G::Fr::from_repr(<G::Fr as PrimeField>::BigInt::from(read_ts[i] as u64)).unwrap()
            })
            .collect::<Vec<G::Fr>>();
        let addrs_fr = (0..addrs.len())
            .map(|i| {
                G::Fr::from_repr(<G::Fr as PrimeField>::BigInt::from(addrs[i] as u64)).unwrap()
            })
            .collect::<Vec<G::Fr>>();
    ...
    let audit_ts = (0..m)
        .map(|i| G::Fr::from_repr(<G::Fr as PrimeField>::BigInt::from(audit_ts[i] as u64)).unwrap())
        .collect::<Vec<G::Fr>>();
...
pub fn memory_checking<G: Curve>(
    ...
    let init_a = (0..mem.len())
        .map(|i| G::Fr::from_repr(<G::Fr as PrimeField>::BigInt::from(i as u64)).unwrap())
        .collect::<Vec<G::Fr>>();
    let init_hash =

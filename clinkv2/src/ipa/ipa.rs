...
impl<G: Curve, D: Digest> InnerProductArgPC<G, D> {
    ...
    fn cm_commit(
        ...
        let mut comm = G::vartime_multiscalar_mul(scalars, comm_key);
    ...
    fn inner_product(l: &[G::Fr], r: &[G::Fr]) -> G::Fr {
        cfg_iter!(l).zip(r).map(|(li, ri)| *li * ri).sum()
    }
    ...
    fn succinct_check<'a>(
        ...
        for (l, r) in l_iter.zip(r_iter) {
            round_challenge =
                Self::compute_random_oracle_challenge(&to_bytes![round_challenge, l, r].unwrap());
    ...
    fn shift_polynomial(
        ck: &CommitterKey<G>,
        p: &DensePolynomial<G::Fr>,
        degree_bound: usize,
    ) -> DensePolynomial<G::Fr> {
        if p.is_zero() {
            ...
            DensePolynomial::from_coefficients_vec(shifted_polynomial_coeffs)
        }
    }
    ...
    pub fn commit<'a>(
        ck: &CommitterKey<G>,
        polynomials: impl IntoIterator<Item = &'a DensePolynomial<G::Fr>>,
        hiding_bound: usize,
        degree_bound: usize,
        ...
        Randomness<G>: 'a,
    {
        ...
        let mut rng = rng.expect("hiding commitments require randomness");
        let mut hiding_polynomial = DensePolynomial::rand(d, &mut rng);
        hiding_polynomial -=
            &DensePolynomial::from_coefficients_slice(&[hiding_polynomial.evaluate(&point)]);
        ...
        combined_commitment_proj +=
            &(hiding_commitment_proj.mul(hiding_challenge.into()) - &ck.s.mul(combined_rand));

...
fn test_plonk() {
    ...
    let new_vk = zkp_plonk::VerifierKey::<Fr, PC>::deserialize_unchecked(&vk_bytes[..]).unwrap();
    //assert_eq!(vk, new_vk);
    ...
    let new_proof = zkp_plonk::Proof::<Fr, PC>::deserialize_unchecked(&proof_bytes[..]).unwrap();

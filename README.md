...
Smart contracts that run as zero-knowledge proof verifiers on the [Nervos CKB chain](https://www.nervos.org/). CKB developers and users can implement various complex zero-knowledge verification processes through the simplest contract invocation. Cooperate with [zkp-toolkit](./README-zkp.md) to complete off-chain prove and on-chain verify.
...
1. Go to _./cli_ and generate a vk file and a proof file using `zkp-toolkit`.

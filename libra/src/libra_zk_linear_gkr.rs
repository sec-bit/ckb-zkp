...
use crate::commitment::{EqProof, LogDotProductProof, ProductProof};
use crate::evaluate::{
...
impl<G: Curve> ZKLinearGKRProof<G> {
    pub fn prover<R: Rng>(
        ...
        for d in (1..circuit.depth).rev() {
            ...
            transcript.append_message(b"comm_x", &to_bytes!(comm_x).unwrap());
            // phase2
            ...
            transcript.append_message(b"comm_y", &to_bytes!(comm_y).unwrap());
    ...
    pub fn verify(
        ...
        for (d, lproof) in self.proofs.iter().enumerate() {
            ...
            transcript.append_message(b"comm_x", &to_bytes!(lproof.comm_x).unwrap());
            ...
            transcript.append_message(b"comm_y", &to_bytes!(lproof.comm_y).unwrap());

...
impl<G: Curve> SumCheckEvalProof<G> {
    pub fn prover<R: Rng>(
        ...
        coeffs[0] += &w[0];
        transcript.append_message(b"Cx", &to_bytes!(comm_poly).unwrap());
        ...
        transcript.append_message(b"Cy", &to_bytes!(comm_claim_value).unwrap());
    ...
    pub fn verify(
        ...
        transcript.append_message(b"Cy", &to_bytes!(comm_claim_value).unwrap());
        transcript.append_message(b"delta", &to_bytes!(self.d_commit).unwrap());
        transcript.append_message(b"beta", &to_bytes!(self.dot_cd_commit).unwrap());
        let mut buf = [0u8; 32];
        transcript.challenge_bytes(b"c", &mut buf);
        let c = random_bytes_to_fr::<G>(&buf);
        let mut coeffs = Vec::new();

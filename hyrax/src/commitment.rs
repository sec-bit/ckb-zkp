...
impl<G: Curve> EqProof<G> {
    ...
    pub fn verify(
        ...
        transcript.append_message(b"C1", &to_bytes!(comm1).unwrap());
        transcript.append_message(b"C2", &to_bytes!(comm2).unwrap());
        transcript.append_message(b"alpha", &to_bytes!(self.alpha).unwrap());
        let mut buf = [0u8; 32];
        transcript.challenge_bytes(b"c", &mut buf);
        let c = random_bytes_to_fr::<G>(&buf);
...
impl<G: Curve> KnowledgeProof<G> {
    pub fn prover<R: Rng>(
        ...
        transcript.append_message(b"C", &to_bytes!(claim_comm).unwrap());
        let t_comm = poly_commit_vec::<G>(&params.generators, &vec![t1], &params.h, t2);
        transcript.append_message(b"alpha", &to_bytes!(t_comm).unwrap());
        let mut buf = [0u8; 31];
        transcript.challenge_bytes(b"c", &mut buf);
        let c = random_bytes_to_fr::<G>(&buf);
        let z1 = claim * &c + &t1;
    ...
    pub fn verify(
        ...
        transcript.append_message(b"C", &to_bytes!(claim_comm).unwrap());
        transcript.append_message(b"alpha", &to_bytes!(self.t_comm).unwrap());
        let mut buf = [0u8; 31];
        transcript.challenge_bytes(b"c", &mut buf);
        let c = random_bytes_to_fr::<G>(&buf);
...
impl<G: Curve> ProductProof<G> {
    pub fn prover<R: Rng>(
        ...
        transcript.append_message(b"X", &to_bytes!(comm_x).unwrap());
        let comm_y = poly_commit_vec::<G>(&params.generators, &vec![claim_y], &params.h, blind_y);
        transcript.append_message(b"Y", &to_bytes!(comm_y).unwrap());
        let comm_prod =
            poly_commit_vec::<G>(&params.generators, &vec![prod], &params.h, blind_prod);
        transcript.append_message(b"Z", &to_bytes!(comm_prod).unwrap());
        ...
        transcript.append_message(b"alpha", &to_bytes!(comm_alpha).unwrap());
        let comm_beta = poly_commit_vec::<G>(&params.generators, &vec![b3], &params.h, b4);
        transcript.append_message(b"beta", &to_bytes!(comm_beta).unwrap());
        let comm_delta = poly_commit_vec::<G>(&vec![comm_x], &vec![b3], &params.h, b5);
        transcript.append_message(b"delta", &to_bytes!(comm_delta).unwrap());
    ...
    pub fn verify(
        ...
        transcript.append_message(b"Y", &to_bytes!(comm_y).unwrap());
        transcript.append_message(b"Z", &to_bytes!(comm_prod).unwrap());
        transcript.append_message(b"alpha", &to_bytes!(self.comm_alpha).unwrap());
        transcript.append_message(b"beta", &to_bytes!(self.comm_beta).unwrap());
        transcript.append_message(b"delta", &to_bytes!(self.comm_delta).unwrap());
...
impl<G: Curve> DotProductProof<G> {
    pub fn prover<R: Rng>(
        ...
        transcript.append_message(b"Cx", &to_bytes!(comm_x).unwrap());

        let comm_y =
            poly_commit_vec::<G>(&params.gen_1.generators, &vec![y], &params.gen_1.h, blind_y);
        transcript.append_message(b"Cy", &to_bytes!(comm_y).unwrap());

        let delta =
            poly_commit_vec::<G>(&params.gen_n.generators, &d_vec, &params.gen_n.h, r_delta);
        transcript.append_message(b"delta", &to_bytes!(comm_x).unwrap());
        ...
        transcript.append_message(b"beta", &to_bytes!(comm_y).unwrap());
    ...
    pub fn verify(
        ...
        transcript.append_message(b"Cx", &to_bytes!(comm_x).unwrap());
        transcript.append_message(b"Cy", &to_bytes!(comm_y).unwrap());
        transcript.append_message(b"delta", &to_bytes!(self.delta).unwrap());
        transcript.append_message(b"beta", &to_bytes!(self.beta).unwrap());
...
impl<G: Curve> LogDotProductProof<G> {
    ...
    pub fn verify(
        ...
        let lhs = (gamma_hat.mul(c) + &self.beta.into_projective()).mul(a_hat.into())
            + &self.delta.into_projective();
        let rhs = (g_hat.into_projective() + &params.gen_1.generators[0].mul(a_hat))
            .mul(self.z1.into())
            + &(params.gen_1.h.mul(self.z2.into()));
...
impl<G: Curve> BulletReduceProof<G> {
    pub fn prover(
        ...
        while n > 1 {
            ...
            // let L: G::G1Projective = G::vartime_multiscalar_mul(
            //     al.to_vec().append(&mut vec![cl, *blind_l]),
            //     &gr.to_vec().append(&mut vec![Q, H]),
            // );
            // let R: G::G1Projective = G::vartime_multiscalar_mul(
            //     ar.to_vec().append(&mut vec![cr, *blind_r]),
            //     &gl.to_vec().append(&mut vec![Q, H]),
            // );
    ...
    pub fn verify(
        ...
        for i in 0..lg_n {
            transcript.append_message(b"L", &to_bytes!(self.l_vec[i]).unwrap());
            transcript.append_message(b"R", &to_bytes!(self.r_vec[i]).unwrap());
            // V challenge x
            let mut buf_x = [0u8; 31];
            transcript.challenge_bytes(b"x", &mut buf_x);
            let x = random_bytes_to_fr::<G>(&buf_x);
